...
¿Reactivar?`
    ...
    try {
      ...
      setPedidos((actual) => actual.map((p) => p.id === pedido.id ? { ...p, ...pedidoActualizado } : p));
      setPedidosMes((actual) => actual.map((p) => p.id === pedido.id ? { ...p, ...pedidoActualizado } : p));

...
function Panel({ negocioInicial, email }) {
    ...
    // ---- Catálogo en PDF ------------------------------------------------
    // Portada + artículos activos, para que la dueña lo tenga descargado y
    // lo pueda mostrar o mandar por WhatsApp sin conexión. Genera el PDF de
    // verdad con jsPDF (vendorizado en vendor/, igual que React: nada de
    // CDN en vivo, la conexión cubana no perdona) en vez de pasar por el
    ...
    async function descargarCatalogoPDF() {
        ...
        try {
            ...
            // ---- Portada ----------------------------------------------
            doc.setFillColor(104, 24, 49); // burgundy de la marca
            ...
            if (logo) {
                ...
                try {
                    ...
                    yPortada += tamLogo + 14;
                } catch (e) { console.warn('[Panel] logo no soportado en el PDF:', e); }
            }
            ...
            // ---- Artículos: una fila por artículo -----------------------
            // Una sola columna a propósito: sin coordenadas de columna que
            // cuadrar, menos riesgo de que algo se superponga.
            ...
            conFoto.forEach((p) => {
                ...
                if (p._foto) {
                    try {
                        doc.addImage(p._foto, formatoImagenPDF(p._foto), margen, y, fotoAncho, fotoAlto, undefined, 'FAST');
                    } catch (e) { console.warn('[Panel] foto no soportada en el PDF:', p.nombre, e); }
                } else {
                    ...
                    {pestana === 'config' && (
                        <>
                            ...
                            <div className="admin-card settings-form">
                                ...
                                <label className="wide">Política ante roturas o extravíos
                                    <textarea placeholder="Ej. La clienta es responsable de reponer o pagar cualquier artículo roto, manchado o extraviado durante el alquiler."
                                        value={negocio.politica_seguro || ''}
                                        onChange={(e) => setNegocio({ ...negocio, politica_seguro: e.target.value })} />
                                    <small>Aparece en el mensaje de WhatsApp de la solicitud, cerca del anticipo. Si la dejas vacía, no se muestra ninguna línea.</small>
                                </label>
                                ...
                                <label className="wide">Mensaje de solicitud (de la clienta a ti)
                                    ...
                                    <small>Variables disponibles: {'{nombre}'}, {'{fechas}'} (el día del evento), {'{items}'}, {'{total}'}, {'{anticipo}'}, {'{politica_seguro}'}, {'{tarjeta}'}, {'{telefono_pago}'}, {'{telefono}'}, {'{notas}'}, {'{pedido_id}'}. {'{telefono}'}, {'{notas}'}, {'{anticipo}'} y {'{politica_seguro}'} ya vienen con su propio emoji y desaparecen del todo si el dato no aplica — ponlas en su propia línea. Es el mensaje que le llega a WhatsApp cuando una clienta pide un alquiler desde tu tienda.</small>
                                </label>
